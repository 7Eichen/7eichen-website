...
function formatEventDate(start: string, end?: string): string {
  ...
  return new Date(start).toLocaleDateString('de-CH', { day: '2-digit', month: 'long', year: 'numeric' })
}
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <p className="font-semibold text-lg" style={{ color: 'var(--foreground)' }}>{event.name}</p>
        <p className="text-sm mt-0.5" style={{ color: 'var(--muted-foreground)' }}>{event.location}</p>
        {event.description && (
          <p className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>{event.description}</p>
        )}
        <p className="text-sm mt-1 font-medium" style={{ color: 'var(--muted-foreground)' }}>{event.organizer}</p>
      </div>
      {event.url && (
